...
int get_ix_pos(int ix, ull incr, int vsize) {
	...
	ull wraps1 = incr / vsize;
	int modulo  = ix + (incr % vsize);
	int wraps2 = modulo / vsize;
	int rem = modulo % vsize;


	return get_ix_pos(rem, wraps1+wraps2, vsize);

}

int get_ix_neg(int ix, ull incr, int vsize) {
	if (ix+incr<= ix && ix+incr >=0 ) {
