...
struct Rock {
	...
	int main() {
		...
		cout<<"ix,rocki,si,maxheight"<<endl;
		...
		while (true) {
			...
			if(!can_move) {
				...
				if (fline)
				{
					...
					/*cout<<"ZOMG!!!"<<fline<<"at rock idx"<<rocki<<" and at si "<<si<<" out of "<<llen<<endl;
					cout<<"Height now, full row filled"<<max_height<<endl;
					for (int k=0;k<7;k++)
						assert(!board.contains(pt(max_height, k)));
					cout<<"Points in rock:"<<endl;
					for(pt &pt1: curr.pos) {
						cout<<"("<<pt1.fi<<","<<pt1.se<<"),";

					}
					cout<<endl;*/
					cout<<ix<<","<<rocki<<","<<si<<","<<max_height<<endl;
					if (ix>1) {
						assert(si == ix * prevsi + (ix-1)*905);
						assert(max_height == ix * prevmax + (ix-1)*272);
						assert(rocki == ix * prevrocki + (ix-1)*160);
					...
					for (int j=0;j<7;j++) {
						...
						board.insert(pt(max_height-1, j));

					}
				}
				/*if (rocki % 1000==0)
					cout<<"we are at rock"<<rocki<<endl;
					*/
				rocki++;
				if (rocki == limit){
