...
// Using abseil's built-in hash for better mixing/dispersion
// struct StateHash - removed, abseil handles it!
...
tuple<State, bool, bool> tryy(Machine &m, const State &s, const vector<int> &button, int times, int currentOpIdx, const vector<int>& lastOpForButton) {
    ...
    uint64_t part1 = 0;
    uint32_t part2 = 0;
    int buttonIdx = getButtonIndex(s) + 1;

    for (int i = 0; i < 8 && i < m.targetJolts.size(); i++) {
        part1 |= ((uint64_t)currCounts[i] << (i * 8));
    }

    uint32_t elem8 = (m.targetJolts.size() > 8) ? currCounts[8] : 0;
    uint32_t elem9 = (m.targetJolts.size() > 9) ? currCounts[9] : 0;
    part2 = (buttonIdx << 16) | (elem9 << 8) | elem8;

    State new_s = {part1, part2};
...
int solve(Machine &m) {
    ...
    absl::flat_hash_map<State, int> seen;  // Uses absl::Hash by default
