...
def _dfs(curr, mat, termini, depth, res, vis):
    ...
    for di, dj in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
        ni, nj = curr[0] + di, curr[1] + dj
        if 0 <= ni < len(mat) and 0 <= nj < len(mat[0]) and (ni, nj) not in vis:
...
def bfs2(st, mat, termini):
    ...
    for di, dj in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
        ...
        if 0 <= ni < len(mat) and 0 <= nj < len(mat) and (ni, nj) not in termini:
            origins.append((ni, nj))
    res = []

    for pt in origins:

        #print(f"{pt} out of {origins}")
...
for pt1 in path:
    ...
    #if pt1 == (2, 1):
    #    raise Exception(visited)
    for origin, pt2, dist in visited:
