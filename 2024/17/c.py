...
regs["A"] = 152568992
out = process(152568992, regs, ops)
#print(regs)
print(out)
sys.exit(0)
...
for ai in (2**44, 2**45, 2**46, 2**47, 2**48):
    #ai = 2**pow
    #if ai % 1000 == 0:
    #    print(ai)
    regs['A'] = ai
    out, aas = process(ai, regs, ops, verbose=False)
    ...
    for l in range(8):
        for j in range(8):
            for i in range(8):
                ...
                bad = False
                for r in rs:
                    #print(f"r={r}, compare {out[r-1]} with {ops[r-1]} and {out[-r]} with {ops[-r]}")
                    if not (out[r-1] == ops[r-1]): #and out[-r] == ops[-r]):
                        bad = True
                if not bad:
                    print(f"match for {j},{i}")
                    print(out)
                    print_stitch(num)
                    matches+=1
